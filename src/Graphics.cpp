...
void Graphics::drawTrafficObjects()
{
	...
	for (auto it : _trafficObjects)
	{
		...
		else if (it->getType() == ObjectType::objectVehicle)
		{
			...
			int r{}; // Apparently, the MSVC, I guess, throws an error because this isn't initialized
			r = sqrt(255 * 255 - g * g - r * r); // ensure that length of color vector is always 255
